...
(defgeneric maildir-create-db (store)
  (:method ((store maildir-store))
    (let ((queries
...
(defun store-get-by-uid (store uid)
  (car (dbi:fetch
        (dbi:execute
         (dbi:prepare (maildir-db store)
...
(defmethod store-save-messages ((store maildir-store) messages)
  (loop with db = (maildir-db store)
        with insert-message = (dbi:prepare db "INSERT INTO message (uid, path, mtime) VALUES (?, ?, ?)")
        ...
        for tmpname = (fad:merge-pathnames-as-file (maildir-folder store "tmp/") filename)
        when (and uid body)
          do (dbi:with-transaction db
               (unless (store-get-by-uid store uid)
                 ...
                 (let ((newname (fad:merge-pathnames-as-file
                                 (if seen
                                     (maildir-folder store "cur/")
                                     (maildir-folder store "new/"))
                                 (if seen
                 ...
                 (dbi:execute insert-message (list uid filename mtime))
